...
$query1 = "SELECT AVG(`panelCurrentControl`) as controlCurrent, AVG(`panelCurrentTest`) as testCurrent, AVG(`panelPowerControl`) as controlPower, AVG(`panelPowerTest`) as testPower, DATE_FORMAT(`deviceLocalTime`, '%D %l %p') as hourOfDay from sensordatarecords WHERE accountID = $accountID AND panelCurrentControl >= 0 AND panelCurrentTest >= 0 AND DATE_SUB(UTC_TIMESTAMP,INTERVAL 23 HOUR) <= deviceLocalTime AND TIME(`deviceLocalTime`) BETWEEN '06:00:00' AND '18:00:00' GROUP BY HOUR(`deviceLocalTime`) ORDER BY `deviceLocalTime` ASC;";
